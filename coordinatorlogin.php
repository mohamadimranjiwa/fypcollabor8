...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $username = htmlspecialchars(trim($_POST['username'])); // Changed to email to match coordinators table
    $password = htmlspecialchars(trim($_POST['password']));

    // Prepare SQL to fetch the coordinator data based on email
    $sql = "SELECT * FROM coordinators WHERE username = ?";
    ...
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            if (password_verify($password, $coordinator['password'])) {
                ...
                // Store coordinator information in the session
                $_SESSION['user_id'] = $coordinator['id'];
                $_SESSION['username'] = $coordinator['username'];
                // No role_id in coordinators table, so omitted
                            ...
                            <div class="col-lg-12">
                                <div class="p-5">
                                    <div class="text-center">
                                        <h1 class="h4 text-gray-900 mb-4">Coordinator Login</h1>
                                    </div>
